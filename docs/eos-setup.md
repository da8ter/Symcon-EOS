...
| Lauf bricht mit „Fresh SoC missing“ ab, obwohl der SoC frisch ist | Bug in 0.4.0rc1: Die SoC-Suche (`configrequest.py`, `key_to_lists(..., dropna=False)`) nimmt den jüngsten Messwert-Datensatz, auch wenn er nur einen anderen Key (EV-SoC, Zählerstand) enthält und der Batterie-SoC darin NaN ist. Das Symcon-Modul umgeht das, indem der EOS Server bei jedem anderen Messwert alle bekannten SoC-Werte mit demselben Zeitstempel erneut sendet. Upstream-Fix: `dropna=True`. |
